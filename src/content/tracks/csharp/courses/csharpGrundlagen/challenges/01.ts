...
export const challenge01: CSharpChallenge = {
  ...
  tutorial: `Ein C#-Programm ist wie in jeder anderen Sprache eine Liste von Anweisungen, die der Reihe nach ausgeführt werden — jede endet mit einem Semikolon <code>;</code>. Die einfachste und wichtigste Anweisung ist <code>Console.WriteLine(...)</code>: Sie gibt Text aus und fügt danach automatisch einen Zeilenumbruch an. <code>Console</code> ist eine eingebaute Klasse, <code>.WriteLine(...)</code> einer ihrer Methodenaufrufe — der Punkt zwischen beiden heißt <b>Dot-Syntax</b> und bedeutet "rufe dieses Mitglied von jenem Objekt/Typ auf". Text (in C# <b>string</b> genannt) steht immer in doppelten Anführungszeichen: <pre>Console.WriteLine("Hallo!"); // gibt Hallo! aus, mit Zeilenumbruch danach</pre>Das <code>//</code> am Zeilenende leitet einen <b>Kommentar</b> ein — alles danach bis zum Zeilenende führt C# nicht aus, es ist nur eine Notiz für Menschen (für mehrzeilige Notizen gibt es <code>/* so */</code>). Moderne C#-Dateien brauchen für ein Programm wie dieses keine sichtbare <code>class</code>- oder <code>Main</code>-Hülle — die Anweisungen stehen einfach direkt in der Datei (<b>Top-Level Statements</b>).`,
